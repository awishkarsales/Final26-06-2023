...
//Import apex method 
import fetchContacts from '@salesforce/apex/assignmentListController.fetchContacts';
...
callFromChild(event){
    ...
    dataTableww.push( EventData.dataToPush );
        ...
        for ( let rec of this.records ){
             roc = rec;
            dataTableww.push(roc);
        }
            alert(JSON.stringify(dataTableww));
        var i = 0;
        for ( let rec of this.records ) {
            if(rec.Id === EventData.dataToPush.Id){
                try {
                    alert(JSON.stringify(dataTableww[i]));
